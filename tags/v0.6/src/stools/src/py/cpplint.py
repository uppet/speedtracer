...
class _CppLintState(object):
  ...
  def SetProjectRoot(self, root):
    ...
    Overrides the logic for determining the project's root. This allows
    header guard checks to work properly on arbitrary source trees.

    Args:
      root: A path (relative or absolute) that is to be used as the
            root of the source tree. This is the root for include files.
    """
...
def _SetProjectRoot(root):
  ...
  Args:
    root: A path (relative or absolute) that is to be used as the
          root of the source tree. This is the root for include files.
  """
  _cpplint_state.SetProjectRoot(root)
...
class FileInfo:
  ...
  def RepositoryName(self):
    ...
    # If the user passed a project_root flag on the command line, we will
    # just use what they gave us.
    if self._project_root is not None:
      return fullname[len(self._project_root) + 1:]
...
def GetHeaderGuardCPPVariable(filename):
  ...
  fileinfo = FileInfo(filename, _cpplint_state.project_root)
...
def CheckSpacing(filename, clean_lines, linenum, error):
  ...
  if IsBlankLine(line):
    ...
    #                because those are not usually indented. Also, ignore the
    #                whitespace in extern blocks.
    if (prevbrace != -1 and prev_line[prevbrace:].find('}') == -1
        and prev_line[:prevbrace].find('namespace') == -1
        and prev_line[:prevbrace].find('extern') == -1):
...
def ParseArguments(args):
  ...
  for (opt, val) in opts:
    if opt == '--help':
      ...
      #TODO(knorton): Make sure all files are in the include root.
      if not os.path.exists(val):
        PrintUsage('the include root must be a valid directory')
      project_root = os.path.abspath(val)
